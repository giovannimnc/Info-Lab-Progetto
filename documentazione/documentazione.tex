...
Funzioni:
\begin{itemize}
\item Generazione mappa da $(i,j)$ a $n$ (ordine naturale), e sua inversa.
Deve leggere il file $\texttt{coords.txt}$ e generare una matrice $(N+2)\times (N+2)$ che associ a ogni valore di $(i,j)$ interno il valore $n$. Deve inoltre generare un vettore che faccia l'inverso, cioè associ $(i,j)$ a $n$ (solo per i punti interni).
La matrice deve infine associare agli $(i,j)$ di bordo un valore tra $0$ e $4N+4$, per poter calcolare il valore della condizione al bordo in quel punto.
In realtà gli associo un valore negativo, per poter distinguere facilmente tra punti di bordo e non.

Complessità: $O(N^2)$.

\item Generazione mappa da $(i,j)$ a $m$ (riordinamento), e sua inversa.
Deve leggere il file $\texttt{ordering.txt}$ e fare l'analogo della funzione precedente, usando la matrice definita sopra per passare da $(i,j)$ a $m$ attraverso $n$.
Complessità: $O(N^2)$.

\item Generazione dei coefficienti non nulli della matrice $A$ del sistema ($N^2 \times N^2)$, generazione del termine noto, ed esportazione.
Scelto un ordinamento, prende in input la matrice e il vettore generati dalle funzioni sopra.
Inoltre prende in input un vettore di $4N+4$ componenti che contiene le condizioni al bordo $u_0(x,y)$.
Crea un file $\texttt{A.txt}$ e un file $\texttt{rhs.txt}$ (ne servirà uno per ogni $N$ e per i due ordinamenti).
...
Usando una delle mappe definite sopra, associa a quelli che non sono di bordo (cioè hanno immagine diversa da -1 tramite la mappa; sono almeno 3 e al massimo 5) un indice $k$.
Calcola il valore di $A_{l,k}$ e scrive sul file $\texttt{A.txt}$ la riga $l,k,A_{l,k}$.
Il formato è COO, cioè sto salvando indici e valore solo dei coefficienti non nulli.

Per gli eventuali indici di bordo, prende il valore $u_0(x,y)$ assegnato e lo somma alla variabile $b$ (con opportuno segno e coefficiente).
Infine scrive sul file il valore di $b$.

Complessità: $O(N^2)$, perché il ciclo su $l$ fa $N^2$ iterazioni, e ogni iterazione contiene poche operazioni elementari.
