...
Output: matrice $L$ tale che $A = -LL^T$. Immagini della struttura di $A,L$ (entrate non nulle).

File coinvolti: notebook $\texttt{soluzione\_sistema.ipynb}$ che legge $\texttt{A.txt}$ generato nel modulo precedente ed esegue la fattorizzazione.
...
Funzioni:
\begin{itemize}
\item Importazione della matrice $A$ e salvataggio in formato CSC.
Legge $\texttt{A.txt}$ in formato COO, scarica le tre colonne come vettori, e converte in CSC (con $\texttt{scipy.sparse.csc\_matrix}$).

\item Calcolo di $L$.
Prende $A$ definita sopra e calcola $L$ con la funzione fornita nel testo. Misura il tempo necessario.

\item Struttura di $A,L$.
Calcola il numero di entrate non nulle in $A,L$ e le disegna usando $\texttt{plt.spy}$.
...
Funzioni:
\begin{itemize}
\item Importazione del termine noto.
Legge $\texttt{rhs.txt}$ e scarica il termine noto come array $b$. 

\item Soluzione del sistema lineare.
Risolve prima il sistema triangolare inferiore $Ly = -b$, poi quello triangolare superiore $L^T u= y$, con la funzione apposita di $\texttt{scipy}$.
L'output è il vettore $u$.
Misura il tempo necessario per risolvere il sistema.

\item Conversione da ordine naturale ($n$) o riordinato ($m$) a coordinate $(i,j)$.
Legge i file $\texttt{coords.txt}$ e $\texttt{ordering.txt}$ per assegnare i valori contenuti nel vettore $u$ ai punti $(i,j)$.
Fa la stessa cosa della funzione $\texttt{GeneraMappaNaturale/Ordinata}$ del modulo "approssimazione lineare", ma solo in un verso.

\item Grafico della soluzione. Prende il vettore soluzione $u$ in un certo ordinamento, e disegna la soluzione. 
\end{itemize}
