...
Infine aggiunge il separatore $V_S$ (anch'esso non salvato) in fondo al vettore dei nodi riordinati.

Complessità: $O(N^2)$. All'interno di ogni chiamata della ricorsione, se il numero di nodi coinvolti è $n$, si scorre su $V_s$ che ha lunghezza $O(\sqrt{n})$. Si parte da $n = N^2$ e a ogni passo si dividono i nodi circa in 2 parti uguali a cui applicare di nuovo l'algoritmo, quindi il tempo necessario è dell'ordine di:
$$\sum_{k=0} ^{\log_2 n} 2^k \sqrt{\frac{n}{2^k}} = \sqrt{n} \sum_{k=0} ^{\log_2 n} 2^{k/2} = \sqrt{n} \frac{2^{(\log_2 n +1)/2} - 1}{2^{1/2} - 1} = O(\sqrt{n} \cdot 2^{(\log_2 n +1)/2}) = O(n) = O(N^2) .$$

\item $\texttt{EsportaOrdinamento}$.
Crea il file $\texttt{ordering.txt}$ e scorre sul vettore dei nodi riordinati, generato da una delle due funzioni sopra.
