...
Se il vicino non è di bordo, aggiunge alla lista di adiacenza (vector di $N^2$ vector, inizializzato nel main) i due nodi coinvolti.
...
Legge $\texttt{A.txt}$ in formato COO, scarica le tre colonne come array (con $\texttt{np.loadtxt}$), e converte in CSC (con $\texttt{scipy.sparse.csc\_matrix}$).
Restituisce la matrice $A$ in CSC.
Complessità: $O(N^2)$, che è il numero di entrate non nulle in $A$.

\item $\texttt{CalcolaCholesky}$.
Prende in input una matrice $A$ (definita negativa) e calcola $L$ (tale che $LL^T = -A)$ con la funzione fornita nel testo.
...
Complessità: $O(nnz(L))$, perché i due sistemi sono triangolari. $nnz(L) = O(N^3)$ per l'ordinamento naturale, $O(N^2 \log N)$ per il riordinamento.
...
In un grafico bilogaritmico di $t$ vs $N$, si osserva una pendenza di circa 3.2 nel caso naturale, 2.2 (escludendo il primo punto un po' anomalo) nel caso riordinato.
Non coincide del tutto con le attese, forse non siamo ancora nel limite asintotico?
...
Il tempo necessario a risolvere il sistema dovrebbe essere circa proporzionale al numero di coefficienti non nulli in $L$, e in effetti dal grafico di $nnz(L)$ vs $N$ si trovano circa le stesse pendenze. 
